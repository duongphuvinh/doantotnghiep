...
def make_sample(label: str, seed: int, size: int) -> Image.Image:
    ...
    if label == "fracture":
        ...
        length = rng.randint(size // 4, size // 3)
        x0 = size // 2 - length // 2
        x1 = size // 2 + length // 2
        y0 = int(y - math.sin(angle) * length // 2)
        y1 = int(y + math.sin(angle) * length // 2)
        draw.line([x0, y0, x1, y1], fill=rng.randint(5, 25), width=rng.randint(5, 9))
        draw.line([x0, y0 - 4, x1, y1 - 4], fill=rng.randint(220, 255), width=2)
    ...
    image = add_noise(image, rng, sigma=rng.uniform(5, 12))
    return image.filter(ImageFilter.GaussianBlur(radius=rng.uniform(0.0, 0.7)))
...
def main() -> None:
    ...
    for label in LABELS:
        ...
        for index in range(args.samples_per_class):
            image = make_sample(label, seed=args.seed + index + hash(label) % 10_000, size=args.size)

...
export function registerToolTask(mcpServer: McpServer) {
  /**
   * NOTE (no-LLM mode):
   * Trước đây tool `medical_record` dùng Gemini để sinh SQL => sẽ tốn quota và dễ lỗi 429.
   * Bản này chuyển sang sinh SQL theo rule đơn giản (deterministic) và dùng query params ($1) để an toàn.
   */
  mcpServer.registerTool(
    'medical_record',
    {
      ...
      description:
        'Công cụ truy vấn bảng thong_tin_dieu_tri (mã người bệnh, họ tên, ngày khám/điều trị, chẩn đoán, kết quả điều trị).',
    ...
    async ({ message }) => {
      ...
      // 1) Nếu có mã người bệnh dạng BN014, BN001...
      const codeMatch = raw.match(/\bBN\d{1,}\b/i);
      if (codeMatch) {
        const code = codeMatch[0].toUpperCase();
        const rows = await query(
          `SELECT * FROM thong_tin_dieu_tri
           WHERE manguoibenh ILIKE '%' || $1 || '%'
      ...
      // 2) Nếu hỏi theo chẩn đoán / kết quả điều trị
      // Ví dụ: "tìm bệnh cảm", "chẩn đoán viêm phổi", "kết quả xuất viện"
      const diagKeywords = ['chẩn đoán', 'chan doan', 'bệnh', 'benh'];
      const resultKeywords = ['kết quả', 'ket qua'];

      const isDiagQuery = diagKeywords.some((k) => q.includes(k));
      const isResultQuery = resultKeywords.some((k) => q.includes(k));

      // 3) Mặc định: coi phần còn lại là họ tên người bệnh
      // Loại bỏ các prefix phổ biến: "người bệnh", "thông tin người bệnh", ...
      const nameOrTerm = raw
        .replace(/thông tin người bệnh\s*/i, '')
        .replace(/thong tin nguoi benh\s*/i, '')
        .replace(/người bệnh\s*/i, '')
        .replace(/nguoi benh\s*/i, '')
        .replace(/^\s*:\s*/i, '')
        .trim();

      // Nếu rỗng thì trả thông báo
      if (!nameOrTerm) {
        return {
          content: [
            {
              type: 'text',
              text: 'Vui lòng nhập: "người bệnh <họ tên>" hoặc "người bệnh BNxxx>".',
            },
          ],
        };
      }

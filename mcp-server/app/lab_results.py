...
NUMERIC_REFERENCES: tuple[NumericReference, ...] = (
    ...
    NumericReference("blood", "CRE", "Creatinine", ("cre", "crea", "creat", "creatinine", "creatinin", "cretinine", "creatinine mau", "creatinin mau", "creatinin máu"), "umol/L", 45.0, 110.0, 250.0, None, "", "Creatinine cao có thể gợi ý giảm chức năng thận hoặc mất nước."),
    ...
    NumericReference("blood", "ALT", "ALT/GPT", ("alt", "gpt", "sgpt", "alat", "alat/gpt"), "U/L", None, 40.0, 200.0, None, "", "ALT tăng có thể liên quan tổn thương tế bào gan, thuốc, rượu hoặc viêm gan."),
    NumericReference("blood", "AST", "AST/GOT", ("ast", "got", "sgot", "asat", "asat/got"), "U/L", None, 40.0, 200.0, None, "", "AST tăng cần đối chiếu gan, cơ, tim và bối cảnh lâm sàng."),
    ...
    NumericReference("blood", "CHOL", "Cholesterol toàn phần", ("chol", "cho", "cholesterol", "choloesterol", "cholesterol tp", "cholesterol toan phan", "cholesterol toàn phần", "total cholesterol"), "mmol/L", None, 5.2, 7.8, None, "", "Cholesterol cao cần đánh giá nguy cơ tim mạch tổng thể và chế độ ăn/thuốc."),
    ...
    NumericReference("blood", "TG", "Triglycerid", ("tg", "trig", "trigly", "triglycerid", "triglyceride", "triglycerides"), "mmol/L", None, 1.7, 5.6, None, "", "Triglycerid cao cần đối chiếu tình trạng nhịn ăn, chuyển hóa đường-mỡ và nguy cơ tim mạch."),
    NumericReference("blood", "HDL", "HDL-C", ("hdl", "hdl c", "hdl-c", "hdl cholesterol", "hdl chol", "cholesterol hdl"), "mmol/L", 1.0, None, None, 0.8, "HDL-C thấp là yếu tố bất lợi về nguy cơ tim mạch.", ""),
    NumericReference("blood", "NONHDL", "Non-HDL Cholesterol", ("non hdl", "non-hdl", "non hdl cholesterol", "non-hdl cholesterol", "non hdl-c", "non-hdl-c"), "mmol/L", None, 4.1, 5.7, None, "", "Non-HDL cholesterol cao cần đánh giá nguy cơ xơ vữa và mục tiêu điều trị theo nguy cơ tim mạch."),
    NumericReference("blood", "LDL", "LDL-C", ("ldl", "ldl c", "ldl-c", "ldl cholesterol", "cholesterol ldl"), "mmol/L", None, 3.4, 4.9, None, "", "LDL-C cao cần đánh giá nguy cơ tim mạch và mục tiêu điều trị cá thể."),
    NumericReference("blood", "UA", "Acid uric", ("ua", "uric acid", "acid uric", "axit uric", "dinh luong acid uric", "định lượng acid uric", "dinh lượng acid uric"), "umol/L", 150.0, 420.0, 600.0, None, "", "Acid uric cao cần đối chiếu gout, chức năng thận, thuốc và chế độ ăn."),
    NumericReference("blood", "MCV", "MCV", ("mcv", "mean corpuscular volume"), "fL", 80.0, 100.0, None, None, "MCV thấp thường gặp trong thiếu máu hồng cầu nhỏ, cần đối chiếu sắt/ferritin và Hb.", "MCV cao cần đối chiếu B12/folate, gan, rượu, thuốc và bệnh nền."),
    NumericReference("blood", "MCH", "MCH", ("mch", "mean corpuscular hemoglobin"), "pg", 27.0, 32.0, None, None, "MCH thấp thường đi cùng hồng cầu nhược sắc.", "MCH cao cần đối chiếu MCV và nguyên nhân hồng cầu to."),
    NumericReference("blood", "MCHC", "MCHC", ("mchc", "mean corpuscular hemoglobin concentration"), "g/dL", 32.0, 36.0, None, None, "MCHC thấp gợi ý hồng cầu nhược sắc.", "MCHC cao cần kiểm tra lại mẫu và đối chiếu huyết học."),
    NumericReference("blood", "RDW", "RDW", ("rdw", "rdw-cv", "red cell distribution width"), "%", 11.5, 14.5, None, None, "", "RDW cao gợi ý kích thước hồng cầu không đồng đều, cần đối chiếu thiếu máu và vi chất."),
    NumericReference("blood", "MPV", "MPV", ("mpv", "mean platelet volume"), "fL", 7.5, 12.0, None, None, "MPV thấp cần đối chiếu số lượng tiểu cầu và bối cảnh lâm sàng.", "MPV cao cần đối chiếu số lượng tiểu cầu và tình trạng viêm/tủy xương."),
    NumericReference("blood", "PDW", "PDW", ("pdw", "platelet distribution width"), "%", 9.0, 17.0, None, None, "PDW thấp cần đối chiếu tiểu cầu và mẫu xét nghiệm.", "PDW cao gợi ý độ phân bố tiểu cầu tăng, cần đối chiếu lâm sàng."),
    NumericReference("blood", "PCT", "PCT tiểu cầu", ("pct", "plateletcrit"), "%", 0.1, 0.5, None, None, "PCT thấp cần đối chiếu số lượng tiểu cầu.", "PCT cao cần đối chiếu số lượng tiểu cầu và bối cảnh viêm/tăng sinh."),
    NumericReference("blood", "HBA1C", "HbA1c", ("hba1c", "hb a1c", "a1c", "hemoglobin a1c"), "%", None, 5.7, 8.0, None, "", "HbA1c cao cần đánh giá nguy cơ hoặc kiểm soát đái tháo đường theo hướng dẫn lâm sàng."),
    NumericReference("blood", "DBIL", "Bilirubin trực tiếp", ("dbil", "direct bilirubin", "bilirubin truc tiep", "bilirubin trực tiếp"), "umol/L", None, 5.0, 20.0, None, "", "Bilirubin trực tiếp tăng cần đối chiếu vàng da, gan mật và các men gan."),
    NumericReference("blood", "IBIL", "Bilirubin gián tiếp", ("ibil", "indirect bilirubin", "bilirubin gian tiep", "bilirubin gián tiếp"), "umol/L", None, 16.0, 40.0, None, "", "Bilirubin gián tiếp tăng cần đối chiếu tan máu, gan và vàng da."),
...
class LabResultProcessor:
    ...
    def _analyze_numeric(self, value: LabValueInput, ref: NumericReference, number: float) -> LabResultItem:
        ...
        return LabResultItem(
            ...
            body_system=ref.body_system or self._body_system_for_code(ref.code),
        )
    ...
    def _analyze_qualitative(self, value: LabValueInput, ref: QualitativeReference) -> LabResultItem:
        ...
        return LabResultItem(
            ...
            severity=severity,
            interpretation=interpretation,
            body_system=self._body_system_for_code(ref.code),
        )
    ...
    def _parse_raw_text(self, raw_text: str, gender: Gender = "unknown") -> list[LabValueInput]:
        ...
        lines = [
            ...
            if (primary := self._primary_report_line(line.strip()))
        ]
    ...
    def _is_supplementary_report_line(self, line: str) -> bool:
        return bool(re.match(r"^\s*\.", line))

    def _primary_report_line(self, line: str) -> str:
        clean = line.strip()
        if not clean or self._is_supplementary_report_line(clean):
            return ""
        return re.split(r"\s+\.\s*(?=[A-Za-zÀ-ỹ])", clean, maxsplit=1)[0].strip()
    ...
    def _find_unrecognized_lines(self, raw_text: str, parsed_inputs: list[LabValueInput]) -> list[str]:
        ...
        for line in raw_text.splitlines():
            clean = self._primary_report_line(line.strip())
            ...
            if any(name and name in normalized for name in recognized_names):
                continue
            reparsed = self._parse_lab_line(clean)
            if reparsed and (
                self._find_numeric_reference(reparsed.name, self._guess_category(reparsed.name))
                or self._find_qualitative_reference(reparsed.name, self._guess_category(reparsed.name))
            ):
                continue
            if self._looks_administrative_line(normalized):
                continue
            if self._is_supplementary_report_line(clean):
                continue
    ...
    def _parse_lab_line(self, clean: str, gender: Gender = "unknown") -> ParsedLabLine | None:
        ...
        if egfr is not None:
            return egfr
        known_numeric = self._parse_known_numeric_lab_line(clean, gender)
        if known_numeric is not None:
            return known_numeric
        match = re.match(
    ...
    def _parse_known_numeric_lab_line(self, clean: str, gender: Gender = "unknown") -> ParsedLabLine | None:
        ref = self._find_numeric_reference(clean, self._guess_category(clean))
        if ref is None:
            return None

        numbers = list(re.finditer(r"(?<![A-Za-z])([+-]?\d+(?:[.,]\d+)?)", clean))
        if not numbers:
            return None

        reference_spans = self._reference_number_spans(clean)
        candidates = [match for match in numbers if not self._span_inside_any(match.span(), reference_spans)]
        if not candidates:
            return None

        value_match = candidates[0]
        unit = self._unit_after_value(clean[value_match.end():]) or ref.unit or None
        reference = self._parse_reference_range(clean, gender)
        return ParsedLabLine(name=ref.display_name, value=value_match.group(1), unit=unit, **reference)

    def _reference_number_spans(self, text: str) -> list[tuple[int, int]]:
        range_pattern = r"([+-]?\d+(?:[.,]\d+)?)\s*(?:-|–|—|to|đến|den)\s*([+-]?\d+(?:[.,]\d+)?)\s*([A-Za-zÀ-ỹ/%^0-9µμ.]+)?"
        comparison_pattern = r"(?:<|<=|≤|>|>=|≥|duoi|dưới|nho hon|nhỏ hơn|tren|trên|lon hon|lớn hơn)\s*([+-]?\d+(?:[.,]\d+)?)\s*([A-Za-zÀ-ỹ/%^0-9µμ.]+)?"
    ...
    def _span_inside_any(self, span: tuple[int, int], containers: list[tuple[int, int]]) -> bool:
        start, end = span
        return any(container_start <= start and end <= container_end for container_start, container_end in containers)

    def _unit_after_value(self, text: str) -> str | None:
        match = re.match(r"\s*([A-Za-zÀ-ỹ/%^0-9µμ.]+)", text)
        if not match:
            return None
        unit = match.group(1).strip()
        if self._to_float(unit) is not None or unit in {"-", "–", "—"}:
            return None
        return unit
    ...
    def _parse_reference_range(self, text: str, gender: Gender = "unknown") -> dict[str, float | str | None]:
        ...
        if range_match:
            ...
            return {
                ...
                "reference_unit": self._clean_reference_unit(unit),
    ...
    def _clean_reference_unit(self, unit: str | None) -> str | None:
        if not unit:
            return None
        clean = unit.strip()
        if self._to_float(clean) is not None:
            return None
        return clean
    ...
    def _find_numeric_reference(self, name: str, category: LabCategory | None) -> NumericReference | None:
        ...
        best: tuple[int, NumericReference] | None = None
        for ref in self.numeric_references:
            ...
            score = self._reference_match_score(normalized, ref.code, aliases)
    ...
    def _find_qualitative_reference(self, name: str, category: LabCategory | None) -> QualitativeReference | None:
        normalized = self._normalize_name(name)
        best: tuple[int, QualitativeReference] | None = None
    ...
    def _matches_reference_name(self, normalized: str, code: str, aliases: tuple[str, ...]) -> bool:
        return self._reference_match_score(normalized, code, aliases) > 0

    def _reference_match_score(self, normalized: str, code: str, aliases: tuple[str, ...]) -> int:
        ...
        score = 0
        if re.search(rf"(?<![a-z0-9]){re.escape(code_normalized)}(?![a-z0-9])", normalized):
            score = max(score, 100 + len(code_normalized))
    ...
    def _body_system_for_code(self, code: str) -> str:
        systems = {
            "GLU": "Đường huyết và chuyển hóa năng lượng",
            "HBA1C": "Kiểm soát đường huyết dài hạn",
            "CRE": "Chức năng thận",
            "EGFR": "Mức lọc cầu thận",
            "UREA": "Chức năng thận và chuyển hóa đạm",
            "UA": "Acid uric, gout và chức năng thận",
            "CHOL": "Mỡ máu và nguy cơ tim mạch",
            "TG": "Mỡ máu và chuyển hóa đường-mỡ",
            "HDL": "Mỡ máu bảo vệ tim mạch",
            "NONHDL": "Mỡ máu gây xơ vữa và nguy cơ tim mạch",
            "LDL": "Mỡ máu và nguy cơ xơ vữa",
            "ALT": "Men gan",
            "AST": "Men gan, cơ và tim",
            "GGT": "Gan mật",
            "ALP": "Gan mật và xương",
            "TBIL": "Gan mật, vàng da và tan máu",
            "DBIL": "Gan mật và đường mật",
            "IBIL": "Tan máu và chuyển hóa bilirubin",
            "ALB": "Dinh dưỡng, gan và thận",
            "TP": "Protein máu, dinh dưỡng và miễn dịch",
            "CRP": "Viêm và nhiễm trùng",
            "WBC": "Miễn dịch và nhiễm trùng",
            "RBC": "Hồng cầu và vận chuyển oxy",
            "HGB": "Thiếu máu và vận chuyển oxy",
            "HCT": "Thể tích hồng cầu và thiếu máu",
            "PLT": "Đông máu và tiểu cầu",
            "MCV": "Kích thước hồng cầu",
            "MCH": "Lượng hemoglobin trong hồng cầu",
            "MCHC": "Nồng độ hemoglobin hồng cầu",
            "RDW": "Độ đồng đều kích thước hồng cầu",
            "MPV": "Kích thước tiểu cầu",
            "PDW": "Độ phân bố tiểu cầu",
            "PCT": "Khối lượng tiểu cầu",
            "NA": "Điện giải và cân bằng nước",
            "K": "Điện giải, tim và cơ",
            "CL": "Điện giải và cân bằng toan kiềm",
            "CA": "Xương, thần kinh và tuyến cận giáp",
            "PHOS": "Xương-khoáng và chức năng thận",
            "IRON": "Sắt và thiếu máu",
            "CK": "Tổn thương cơ",
            "LDH": "Tổn thương mô, gan, cơ và tan máu",
            "SG": "Cô đặc nước tiểu và tình trạng nước",
            "PH": "pH nước tiểu và chuyển hóa",
            "PRO": "Protein niệu và chức năng thận",
            "GLU_U": "Glucose niệu và đường huyết",
            "KET": "Ketone niệu và chuyển hóa năng lượng",
            "BLD": "Máu trong nước tiểu và tiết niệu",
            "LEU": "Bạch cầu niệu và nhiễm trùng tiết niệu",
            "NIT": "Nitrite niệu và nhiễm khuẩn tiết niệu",
        }
        return systems.get(code, "Chức năng cơ thể liên quan")
